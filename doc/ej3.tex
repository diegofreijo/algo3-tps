...
Notar que el tama'no de la entrada ser'a la cantidad de elementos en la matriz. Es decir, $t = 2k^2$. Por lo tanto, la complejidad uniforme es O($\sqrt{t/2}\ t/2$) = O($t^{3/2}$), con lo que es \negrita{superlineal} en funci'on del tama'no de entrada.
...
Luego, se verifica f'acilemnte que
