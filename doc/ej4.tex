...
\imagen{img/Tp1Ej4.png}{14}{Cantidad de operaciones en funci'on del n'umero de entrada.}
\imagen{img/Tp1Ej4(primos).png}{14}{Cantidad de operaciones en funci'on del n'umero de entrada. 
S'olo n'umeros primos.}
\newpage
...
Se contaron la cantidad de operaciones realizadas por Factorizaci'on para entradas desde 2 hasta 500 y, como se puede ver en el gr'afico, la complejidad te'orica supera a la pr'actica a partir de cierto valor (140 aproximadamente).

Por lo tanto la cota te'orica est'a bien calculada, pero puede parecer exagerada. 'Esto es perfectamente esperable debido a que en el c'alculo de la complejidad se consider'o como peor caso de EsPrimo aquel donde $todos$ los valores menores a $\sqrt{n}$ eran primos, y por supuesto 'esto es imposible (excepto, obviamente, con valores peque'nos). De existir una mejor forma para acotar la cantidad de primos menores a $\sqrt{n}$, entonces la complejidad te'orica podr'ia ser mejor ajustada.

Contrastando el primer con el segundo gr'afico (entradas desde 2 a 500 pero solamente primos) se puede ver que, efectivamente, el peor caso es cuando el n'umero a factorizar es primo.
